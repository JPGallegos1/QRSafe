...
7. **Opción 3 — SDK en billeteras: es la única arquitectura que se interpone realmente en el flujo de pago, y hay una palanca regulatoria concreta.** La Com. "A" 8032 establece que *"cuando un pago con tarjeta de crédito se inicie desde una billetera digital interoperable mediante la lectura de un código QR, la responsabilidad por fraude será asumida por la billetera"* [PRIMARIA] **HECHO**: el actor que absorbe la pérdida es el que tiene que integrar la mitigación. Y la Com. "A" 7463 obliga a los esquemas a apoyar sus análisis de fraude con herramientas que identifiquen patrones sospechosos, y a **contemplar** acciones según el riesgo evaluado — nombrando como **ejemplo** *"alertar al cliente ordenante y/o requerirle confirmación por vías alternativas antes de cursar la transacción"* [PRIMARIA] **HECHO**. Es una obligación de medio con un ejemplo que describe la función de QRSafe: legitima la categoría, **no obliga a comprarla** (ver §4.3). Lo que **no** existe hoy es un SDK público de billetera al que enchufarse: el único programa documentado (flujo emisor de Mercado Pago) está reservado a billeteras digitales externas, no a verificadores.
...
| **Cobertura** | **~9,04% confirmado** del navegador móvil argentino (Safari 8,84% + Firefox 0,20%). Chrome, con 86,31%, queda estructuralmente fuera. **El límite superior queda abierto**: Samsung Internet (3,91%) no fue verificado (§3.1.1), y si soportara WebExtensions el total treparía a ~13%. [SECUNDARIA, StatCounter jul-2026]. Cero cobertura del Flujo A. |
...
| **Cobertura del parque AR** | 87,46% (Android) | 12,51% (iOS) | 9,04% confirmado (Safari + Firefox móvil); hasta ~13% si Samsung Internet califica | n/a | 0% hoy; ~100% por billetera integrada |
...
2. **La vía de extensiones cubre ~9% confirmado**, y su límite es estructural más que coyuntural: depende de que Google incorpore extensiones a Chrome Android, algo que su propio tracker sitúa fuera de alcance para móvil. El techo exacto queda abierto hasta verificar Samsung Internet (3,91%), que podría llevarlo a ~13% — sigue siendo una fracción del parque, pero no debe presentarse como cerrado.
