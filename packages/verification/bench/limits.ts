...
async function run(sweep: Sweep): Promise<void> {
  let lastSuccessfulValue: number | null = null;
  const trail: string[] = [];
  ...
  console.log('  ' + label + sweep.phrase(lastSuccessfulValue));
  console.log('  ' + ' '.repeat(22) + 'tested: ' + trail.join(' → '));
}

async function main(): Promise<void> {
  console.log('\nReading limits — one variable at a time, using a generated QR\n');
  console.log('  canvas ' + CANVAS + 'px, base QR ' + BASE_QR + 'px, real Coto payload\n');
  ...
  await run({
    ...
    phrase: (v) => (v === null ? 'cannot read even the largest' : 'reads down to ' + v + ' px per side'),
  });
  ...
  await run({
    ...
    phrase: (v) => (v === null ? 'cannot read even head-on' : 'reads up to ' + v + '°'),
  });
  ...
  await run({
    ...
    phrase: (v) => (v === null ? 'cannot read even when sharp' : 'reads up to a ' + v + ' px radius'),
  });
  ...
  await run({
    ...
    phrase: (v) => (v === null ? 'cannot read even head-on' : 'reads up to ' + Math.round((v ?? 0) * 100) + '% tilt'),
  });
  ...
  await run({
    ...
    phrase: (v) => (v === null ? 'cannot read even when well lit' : 'reads up to ' + Math.round((v ?? 0) * 100) + '% dimming'),
  });

  await run({
    axis: 'JPEG compression',
    unit: 'quality',
    values: [95, 80, 60, 40, 30, 20, 15, 10, 5],
    build: (q) => jpeg(base, q),
    phrase: (v) => (v === null ? 'cannot read even at quality 95' : 'reads down to quality ' + v),
  });

  // A realistic case combines multiple degradations at once.
  console.log('\n  combined — resembling a street photo\n');
  const combos: [string, () => Promise<Image | Buffer>][] = [
    ...
    ['QR 100px + low light 0.3', async () => dim(await onCanvas(PAYLOAD, CANVAS, 100), 0.3)],
  ];
  for (const [name, build] of combos) {
    const ok = await reads(await build());
    console.log('    ' + (ok ? 'reads ' : 'FAILS ') + name);
  }
  console.log('');
}
